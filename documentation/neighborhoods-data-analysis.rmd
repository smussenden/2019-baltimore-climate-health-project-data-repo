...
This R markdown document describes the methodology, results and explanation of a portion of the data analysis we conducted in support of reporting project examining the effects of climate-change driven temperature increases on the health of people who live in cities, focusing on Baltimore.

Read the whole project, "[Code Red: Baltimore's Climate Divide](https://cnsmaryland.org/code-red
)." 

This document provides support for facts in the story, "[In urban heat islands, climate crisis hits harder](https://cnsmaryland.org/interactives/summer-2019/code-red/neighborhood-heat-inequality.html#story)".
...
audrey_day_averages<- read_csv(paste0(path_to_data, "temperature_sensors/audrey/audrey_day_averages.csv"))
audrey_day_hourly_averages <- read_csv(paste0(path_to_data, "temperature_sensors/audrey/audrey_day_hourly_averages.csv"))
audrey_day_minute_averages <- read_csv(paste0(path_to_data, "temperature_sensors/audrey/audrey_day_minute_averages.csv"))
...
op_er_full_zip_correlation_matrix <- read_csv(paste0(path_to_data, "hospital_data/op_er/op_er_full_zip_correlation_matrix.csv"))
op_er_full_zip_disease_heat <- read_csv(paste0(path_to_data, "hospital_data/op_er/op_er_full_zip_disease_heat.csv"))
ip_full_zip_correlation_matrix <- read_csv(paste0(path_to_data, "hospital_data/ip/ip_full_zip_correlation_matrix.csv"))
ip_full_zip_disease_heat <- read_csv(paste0(path_to_data, "hospital_data/ip/ip_full_zip_disease_heat.csv"))
...
Over an 11-day period in July -- July 12 to July 22 -- each day had a max temperature of at least 90 degrees F and max heat indexes of at least 92 degrees F. 
...
“Can’t even put your head out the door,” said Tammy Jackson, 48, on a day when the temperature outside hit 100 degrees and 92 in her home.  “This is too much. Oh Lord, this is too much.”

#### Explanation

Tammy Jackson, a resident of Baltimore's McElderry Park neighborhood, had a temperature and humidity sensor installed in her house by our reporters.  In her home, the highest temperature reading on Saturday, July 20, the day she said that quote to a reporter, registered 92 degrees (91.6 degrees F). For outdoor temperature, in most of the project, we used hourly snapshot readings taken at a National Weather Service monitoring station in the Inner Harbor.  The highest hourly reading in that data set on that day was 99 degrees. But the NWS official daily summary for that monitoring station -- which incorporates all readings taken, not just hourly snapshots -- indicates it did hit 100 degrees at some point on July 20.    
