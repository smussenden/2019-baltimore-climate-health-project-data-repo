...
full_data <- full_data %>%
  mutate(adjusted_heat_index_nws_five_scale_bucket = case_when(
    adjusted_heat_index <= 79 ~ "Under_80",
    adjusted_heat_index >= 80 & adjusted_heat_index <= 89 ~ "80_89",
    adjusted_heat_index >= 90 & adjusted_heat_index <= 99 ~ "90_99",
    adjusted_heat_index >= 100 & adjusted_heat_index <= 124 ~ "100_plus"
  ), adjusted_heat_index_nws_five_scale_bucket_order = case_when(
    adjusted_heat_index_nws_five_scale_bucket == "Under_80" ~ "A",
    adjusted_heat_index_nws_five_scale_bucket == "80_89" ~ "B",
    adjusted_heat_index_nws_five_scale_bucket == "90_99" ~ "C",
    adjusted_heat_index_nws_five_scale_bucket == "100_plus" ~ "D"
  )
  )
...
temp_data <- temp_data %>%
  mutate(heat_index_nws_five_scale_bucket = case_when(
    HEAT_INDEX <= 79 ~ "Under_80",
    HEAT_INDEX >= 80 & HEAT_INDEX <= 89 ~ "80_89",
    HEAT_INDEX >= 90 & HEAT_INDEX <= 99 ~ "90_99",
    HEAT_INDEX >= 100 & HEAT_INDEX <= 124 ~ "100_plus"
  ), heat_index_nws_five_scale_bucket_order = case_when(
    heat_index_nws_five_scale_bucket == "Under_80" ~ "A",
    heat_index_nws_five_scale_bucket == "80_89" ~ "B",
    heat_index_nws_five_scale_bucket == "90_99" ~ "C",
    heat_index_nws_five_scale_bucket == "100_124" ~ "D"
  )
  )
...
call_heat_index_ratio_five_primary_impression_group <- full_data %>%
  ...
  select(primary_impression_group, `Under_80`, `80_89`, `90_99`, `100_plus`)
